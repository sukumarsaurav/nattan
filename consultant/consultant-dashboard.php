...
$consultant_query = "SELECT * FROM consultants WHERE id = $consultant_id";
$consultant_result = executeQuery($consultant_query);
$consultant = mysqli_fetch_assoc($consultant_result);

// Get dashboard statistics
$total_appointments = getRecordCount('appointments', "consultant_id = $consultant_id");
$upcoming_appointments = getRecordCount('appointments', "consultant_id = $consultant_id AND appointment_datetime >= NOW() AND status IN ('pending', 'confirmed')");
$today_appointments = getRecordCount('appointments', "consultant_id = $consultant_id AND DATE(appointment_datetime) = CURDATE()");
$completed_appointments = getRecordCount('appointments', "consultant_id = $consultant_id AND status = 'completed'");

// Get recent appointments
$recent_sql = "SELECT * FROM appointments WHERE consultant_id = $consultant_id ORDER BY created_at DESC LIMIT 5";
