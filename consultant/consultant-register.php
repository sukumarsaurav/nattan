...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $required_fields = ['first_name', 'last_name', 'email', 'password', 'confirm_password', 'rcic_number', 'phone'];
    $missing_fields = [];
    ...
    // Check if email already exists
        ...
        $check_query = "SELECT * FROM consultants WHERE email = '$email'";
        $result = mysqli_query($conn, $check_query);
        
        if (mysqli_num_rows($result) > 0) {
            $error = "This email is already registered. Please use a different email or login.";
        } else {
            // Process the consultant registration
            $first_name = mysqli_real_escape_string($conn, $_POST['first_name']);
            $last_name = mysqli_real_escape_string($conn, $_POST['last_name']);
            $password = password_hash($_POST['password'], PASSWORD_DEFAULT);
            $rcic_number = mysqli_real_escape_string($conn, $_POST['rcic_number']);
            ...
            // Insert the new consultant
            $insert_query = "INSERT INTO consultants (first_name, last_name, email, password, rcic_number, phone, specialization, languages, bio, status, created_at) 
                             VALUES ('$first_name', '$last_name', '$email', '$password', '$rcic_number', '$phone', '$specialization', '$languages', '$bio', '$status', '$created_at')";
            
            if (mysqli_query($conn, $insert_query)) {
                $success = "Registration successful! Your account is pending approval by our team. We will notify you by email once it's approved.";
            } else {
